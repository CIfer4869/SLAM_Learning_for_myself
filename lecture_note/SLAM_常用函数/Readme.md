...
> 本笔记以“库”为主线，先整理 `Eigen` 中与矩阵、几何、位姿相关的常用类型与函数，再在后续章节补充 `Sophus` 的 `SO(3)` / `SE(3)` 李群与李代数写法，并预留后续功能模块目录，便于逐步扩展可视化、图像处理、优化等内容。  
> 参考代码：`lecture_files/lecture_3/useEigen/eigenMatrix.cpp`、`lecture_files/lecture_3/useGeometry/eigenGeometry.cpp`、`lecture_files/lecture_4/useSophus/useSophus.cpp` 及其详细注释。
> 相关库说明：本笔记按库分章：第一部分统一收录 `Eigen` 的矩阵、向量、几何与位姿模块；第二部分单独收录 `Sophus` 的 `SO(3)` 与 `SE(3)` 李群/李代数表示；第三部分再预留 `Pangolin`、`OpenCV`、`Ceres`、`g2o` 等常见 SLAM 生态库的功能入口。
...
- [SLAM 常用函数速查笔记](#slam-常用函数速查笔记)
  ...
  - [一、Eigen 库](#一eigen-库)
    - [1.1 矩阵与向量类型](#11-矩阵与向量类型)
      ...
      - [0. 预备知识：程序入口与命令行参数](#0-预备知识程序入口与命令行参数)
    ...
    - [1.5 几何模块（旋转与变换）](#15-几何模块旋转与变换)
      - [1.5.1 旋转向量 AngleAxis](#151-旋转向量-angleaxis)
      - [1.5.2 旋转矩阵与欧拉角](#152-旋转矩阵与欧拉角)
      - [1.5.3 四元数 Quaternion](#153-四元数-quaternion)
      - [1.5.4 欧氏变换 Isometry3d](#154-欧氏变换-isometry3d)
      - [1.5.5 坐标变换](#155-坐标变换)
  - [二、Sophus 库](#二sophus-库)
    - [2.1 SO(3) 与李代数](#21-so3-与李代数)
      - [2.1.1 用旋转矩阵、旋转向量、四元数构造 SO(3)](#211-用旋转矩阵旋转向量四元数构造-so3)
      - [2.1.2 用 log() 获取李代数，hat() 与 vee() 互换](#212-用-log-获取李代数hat-与-vee-互换)
      - [2.1.3 SO(3) 的增量扰动更新模型](#213-so3-的增量扰动更新模型)
    - [2.2 SE(3) 与李代数](#22-se3-与李代数)
      - [2.2.1 用 SE(3) 同步表示旋转 + 平移](#221-用-se3-同步表示旋转--平移)
      - [2.2.2 任意 SE(3) 对象都能以 log() 得到 6 维李代数](#222-任意-se3-对象都能以-log-得到-6-维李代数)
      - [2.2.3 SE(3) 的 hat() 与 vee() 以及更新方式](#223-se3-的-hat-与-vee-以及更新方式)
    - [2.3 深入理解 Sophus 与 Eigen 的联系](#23-深入理解-sophus-与-eigen-的联系)
...
- `Eigen`：负责基础矩阵、向量、旋转、四元数、欧氏变换等几何表达，是本笔记第一部分最核心的数值库；
- `Sophus`：负责 `SO(3)` 与 `SE(3)` 的李群/李代数表示，主要对应本笔记第二部分的 `Sophus` 库章节；
- 后续功能模块继续留作扩展：`Pangolin` 用于可视化交互，`OpenCV` 用于图像与特征处理，`Ceres` 与 `g2o` 用于非线性优化。

这说明本笔记不是只写 `Eigen` 的单一知识点，而是围绕 SLAM 常用几何建模与位姿表达逐步衔接的“函数速查式教程”结构。按库来读时，核心顺序应为：先学基础矩阵与向量表达，接着进入 `Eigen` 旋转与变换几何模块，再切入 `Sophus` 的李群/李代数语义，最后再接入可视化、图像处理与优化工具链。
...
实际工程中，程序入口总是从 `int main(int argc, char** argv)` 开始，命令行参数则由 `argc` 与 `argv` 记录；这类设置属于工作流预备知识，不是 Eigen 的核心函数内容。
...
实际 SLAM 中多数位姿对象不只停留在 `Eigen::Matrix3d` 或 `Eigen::Isometry3d` 这样的“数学表示”层面，还会进一步抽象为“李群”对象。课程今天的示例文件使用的是 `Sophus` 库，核心对象是：

```cpp
Sophus::SO3d   // 三维旋转群，表示 SO(3)
Sophus::SE3d   // 三维刚体变换群，表示 SE(3)
```

这两个对象都来自 `Sophus` 模板库，内部以四元数、旋转矩阵、平移向量等表示。它们分别对应：

- `SO(3)`：三维旋转群，只有旋转自由度 3；
- `SE(3)`：特殊欧氏群，旋转 + 平移，总自由度 6。

#### 2.1.1 用旋转矩阵、旋转向量、四元数构造 SO(3)
...
Eigen::Quaterniond q(R);                  // 从旋转矩阵得到四元数
Sophus::SO3d SO3_q(q);                   // 从四元数构造
```

这三种类型都是等价表达：

- `SO3_R` 用矩阵表示;
- `SO3_v` 用旋转向量（轴角）表示，`exp()` 实现从李代数 $\omega$ 到群元素的指数映射;
- `SO3_q` 用四元数表示，内核可统一再映射回旋转矩阵。

其中 `Sophus::SO3d::exp()` 会把三维李代数向量构造成一个 `SO3` 群元素，符合李群与李代数之间关系：

$$
R = \exp(\hat{\omega})\qquad \omega \in \mathfrak{so}(3)
$$

#### 2.1.2 用 log() 获取李代数，hat() 与 vee() 互换

Sophus 中对 `SO(3)` 提供了对数映射：
...
这里的 `so3` 就是 `SO(3)` 对应的李代数向量，三维向量形式，表示旋转轴角的“切空间”变量。`SO3d::hat()` 可以把三维向量变成反对称矩阵，而 `SO3d::vee()` 则把反对称矩阵恢复成三维向量：
...
这正是李代数中的“帽子算子”和“鞋子算子”：

- `hat(\omega)`：把三维向量 $\\omega$ 映射成一个反对称矩阵 $\hat{\omega}$；
- `vee(A)`：把反对称矩阵 $A$ 恢复为向量 $\omega$。

#### 2.1.3 SO(3) 的增量扰动更新模型

在优化或者位姿更新中，使用增量扰动一般写成：
...
- `update_so3` 是小增量的李代数向量；
- `Sophus::SO3d::exp(update_so3)` 先把它映射为新的旋转群元素；
- 再用左乘方式把它“附加”到旧旋转对象上：

$$
R_{new} = \exp(\delta \omega) \cdot R
$$

这叫做“左扰动模型”，常用于增量式优化和滤波器更新。它和右乘 `R * exp(\delta \omega)` 形成互补语义，前者视增量写在“当前帧左边”，后者写在“当前帧右边”。

### 2.2 SE(3) 与李代数

#### 2.2.1 用 SE(3) 同步表示旋转 + 平移

`SE(3)` 中的设计与 `SO(3)` 相同，但它额外保存一个三维平移向量：

```cpp
Eigen::Vector3d t(1, 0, 0);
Sophus::SE3d SE3_Rt(R, t);    // 从 R,t 直接构造
Sophus::SE3d SE3_qt(q, t);    // 从 q,t 直接构造
```
...
- `R` 为 3×3 旋转矩阵；
- `t` 为 3×1 平移向量；
- `SE3_Rt` 和 `SE3_qt` 都是 `SE3d` 的实例，二者等价地描述相同的刚体变换。
...
#### 2.2.2 任意 SE(3) 对象都能以 log() 得到 6 维李代数

SE(3) 的李代数 `se(3)` 是 6 维向量，习惯上采用“平移在前，旋转在后”的排列顺序：
...
- 前三维表示平移量（`upsilon` 或 `t` 的增量）；
- 后三维表示旋转量（`omega`）。

这和很多 SLAM 实现中用“平移在前、旋转在后”的习惯写法一致，便于写入优化器、滤波器或者位姿增量模型。

#### 2.2.3 SE(3) 的 hat() 与 vee() 以及更新方式

SE(3) 同样有类似的李代数到矩阵的映射：
...
其中 `se3` 是 6 维切空间向量，`hat()` 讲它映射为一个 4×4 的齐次扰动矩阵形式：
...
而 `vee()` 会把这个矩阵反映回 6 维向量，用于理解 `SE(3)` 的增量结构。类似 `SO(3)`，在 `SE(3)` 中也可用指数映射完成局部扰动更新：
...
$$
T_{new} = \exp(\hat{\xi}) \cdot T
$$

也就是把一次“微小位姿增量”写成一个 `SE(3)` 的左乘更新。它非常适用于相机位姿的增量式估计、状态机更新，以及优化中的局部线性化。

### 2.3 深入理解 Sophus 与 Eigen 的联系

在本节示例中，真实的代码工作流可以理解为：

1. 用 `Eigen::Matrix3d` 和 `Eigen::Quaterniond` 从旋转向量或角轴创建基础旋转表达；
2. 用 `Sophus::SO3d` 和 `Sophus::SE3d` 将它们包装为李群对象；
3. 用 `log()` / `exp()`、`hat()` / `vee()` 进行李代数映射；
4. 用 `matrix()` 输出/保存齐次矩阵；
5. 在优化与估计中，将位姿增量用 `SE3d::exp()` 更新成新的位姿。

也正因如此，`Sophus` 不只是“帮 Eigen 对象加上名称”，而是维护“群结构 + 切空间 + 指数/对数映射”这一整条李群框架，是 SLAM 中最常见的位姿建模工具之一。
